...
(function($){
	...
	$.fn.datepicker = function(opts){
		...
		function keydownInt(e){
			if (!((e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode == Bird.keyCode.DELETE || e.keyCode == Bird.keyCode.BACKSPACE))) { return false; }
		}
		...
		return this.each(function(){
			...
			$this.click(function(event){
				...
				$(Bird.frag['calendarFrag']).appendTo("body").css({
					left:offset.left+'px',
					top:iTop+'px'
			...
			//TODO 解决在同一个parent中的ico错误问题
			var ownIcon = $this.attr("ownIcon");
			if(ownIcon){
				setting.calIcon$ = ownIcon;
			}
