...
(function($){
		...
		checkboxCtrl: function(parent){
			return this.each(function(){
				...
				$trigger.click(function(){
					...
					var other = $trigger.attr("other") || "";
					if ($trigger.is(":checkbox")) {
						var type = $trigger.is(":checked") ? "all" : "none";
						if (group) $.checkbox.select(group, type, parent,other);
					} else {
						if (group) $.checkbox.select(group, $trigger.attr("selectType") || "all",undefined,other);
					}
				});
			});
		}
	...
	$.checkbox = {
		...
		select: function(_name, _type, _parent,other){
			$parent = $(_parent || document);
			$checkboxLi = $parent.find(":checkbox[name='"+_name+"']"+other);
			switch(_type){
				case "invert":
