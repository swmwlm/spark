...
$.pdialog.open(Bird._set.loginUrl,"login",Bird._set.loginTitle,{mask:true,width:520,height:260,restore:false,minable:false});}else{
window.location=Bird._set.loginUrl;}},
loadReg:function(){
if($.pdialog&&Bird._set.regTitle){
$.pdialog.open(Bird._set.pregUrl,"preg",Bird._set.regTitle,{mask:true,width:520,height:260,restore:false,minable:false});}else{
...
if(json.message&&alertMsg)alertMsg.error(json.message);}else if(json.statusCode==Bird.statusCode.timeout){
if(alertMsg)alertMsg.error(json.message||Bird.msg("sessionTimout"),{okCall:Bird.loadLogin});
else Bird.loadLogin();}else if(json.statusCode==Bird.statusCode.noReg){
if(alertMsg)alertMsg.error(json.message,{okCall:Bird.loadLogin});
else Bird.loadLogin();}else{
...
if(viewType=='navTab'){
navTab.closeCurrentTab();}else{
$.pdialog.closeCurrent();}
Bird.loadLogin();}});}else if(json.statusCode==Bird.statusCode.noReg){
alertMsg.error(json.message,{okCall:function(){
var viewType=op.viewType||'navTab';
if(viewType=='navTab'){
navTab.closeCurrentTab();}else{
$.pdialog.closeCurrent();}
Bird.loadReg();}});}else if(json.statusCode==Bird.statusCode.error||json.statusCode==Bird.statusCode.noAuth){
...
$this.focus(function(){
getAltBox().css(altBoxCss(0.1));}).blur(function(){
if(!$(this).val())getAltBox().show().css("opacity",0.5);}).keydown(function(){
getAltBox().hide();});});},
isTag:function(tn){
if(!tn)return false;
...
return(/^(([0\+]\d{2,3}-)?(0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$/.test(this.trim()));}});})(jQuery);
function Map(){
...
accordion[options].apply(accordion,args);}else if(!$(this).is(".bird-accordion"))
...
if(json.statusCode==Bird.statusCode.timeout){
alertMsg.error(json.message||Bird.msg("sessionTimout"),{okCall:function(){
Bird.loadLogin();}});}else if(json.statusCode==Bird.statusCode.error||json.statusCode==Bird.statusCode.noAuth){
if(alertMsg&&json.message)alertMsg.error(json.message);}else{
...
this._closeTimer=setTimeout(function(){alertMsg.close()},2000);}else{
$(this._bgId).show();}
...
if($tab){
if(this._indexTabId(tabid)==this._currentIndex)this._reload($tab,true,true);
...
if($panel){
if(!url){
url=$tab.attr("url");}else{
$tab.attr("url",url);}
if(!op.data){
op.data=$tab.data("data");}else{
...
if($.isFunction(op.callback))op.callback(response);},viewType:"navTab"});}}}},
...
navTab._loadUrlCallback($panel);},viewType:"navTab"});}}
...
this._taskBar=$(Bird.frag["taskbar"]).appendTo($("#layout"));
...
$.taskBar._taskBar.hide();});}}})(jQuery);(function($){
$.fn.extend({
...
if(group)$.checkbox.select(group,type,parent,other);}else{
...
error:Bird.ajaxError});});}});}});})(jQuery);(function($){
$.extend({
History:{
_hash:new Array(),
_cont:undefined,
