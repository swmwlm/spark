	...
	$.fn.extend({
		pagination: function(opts,$p){
			...
			return this.each(function(){
				...
				$this.html(Bird.frag["pagination"].replaceAll("#pageNumFrag#", pageNumFrag).replaceAll("#currentPage#", pc.getCurrentPage())).find("li").hoverClass();
				...
				$this.find(setting.jumpto$).each(function(){
					...
					$button.click(function(event){
						...
						if (pageNum && pageNum.isPositiveInteger()) {
							birdPageBreak({targetType:pc.targetType(), rel:pc.rel(), data: {pageNum:pageNum}},$page);
						}
					});
					$inputBox.keyup(function(event){
						if (event.keyCode == Bird.keyCode.ENTER) $button.click();
					});
				});
			});
			...
			function _bindEvent($target, pageNum, targetType, rel){
				$target.bind("click", {pageNum:pageNum}, function(event){
					birdPageBreak({targetType:targetType, rel:rel, data:{pageNum:event.data.pageNum}},$page);
		...
		orderBy: function(options){
			...
			return this.each(function(){
				var $this = $(this).css({cursor:"pointer"}).click(function(){
					...
					birdPageBreak({targetType:op.targetType, rel:op.rel, data:{orderField: orderField, orderDirection: orderDirection}},$page);
				});
