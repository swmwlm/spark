...
var navTab = {
	...
	//TODO 增加withData参数，代表reload时是否带有参数请求，没有则reload默认页面，有则reload最新页面,update2011-12-02 by lzy
	_reload: function($tab, flag,withData){
		flag = flag || $tab.data("reloadFlag");
		var url = $tab.attr("url");
		var data = {};
		if(withData==true){//获取缓存的请求数据update 2011-12-02 by lzy
			data = $tab.data("data");
		}
		if (flag && url) {
			...
			if ($tab.hasClass("external")){
				...
				$panel.loadUrl(url, data, function(){navTab._loadUrlCallback($panel);},'POST' , "navTab");
			}
		}
	},
	reloadFlag: function(tabid){
		...
		if ($tab){
			if (this._indexTabId(tabid) == this._currentIndex) this._reload($tab, true,true);//TODO 修改为带参数请求 update2011-12-02 by lzy
			else $tab.data("reloadFlag", 1,true);
		}
	},
	reload: function(url, options){//TODO 无任何参数时代表reload本navTab update2011-12-02 by lzy
		var op = $.extend({navTabId:"", callback:null}, options);
		...
		if ($panel){
			...
			//TODO 增加获取缓存请求数据 update 2011-12-02
			if(!op.data){
				op.data = $tab.data("data");
			}else{
				//TODO 将data数据放进tab中以便刷新时继续留在本条件中update 2011-12-02 By lzy
	...
	checkTimeout:function(){
		var json = Bird.jsonEval(this.getCurrentPanel().html());
		if (json && json.statusCode == Bird.statusCode.timeout) this.closeCurrentTab();
	},
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		...
		if (iOpenIndex >= 0){
			...
			this._moreBox.append('<li><a href="javascript:void(0);" title="#title#">#title#</a></li>'.replaceAll("#title#", op.title));
