...
var Bird = {
	...
	keyCode: {
		...
		F1:112,F2:113,F3:114,F4:115,F5:116,F6:117,F8:119,F9:120,F10:121,F11:122,F12:123
	},
	pageInfo: {pageNum:"pageNum", numPerPage:"numPerPage", orderField:"orderField", orderDirection:"orderDirection"},
	statusCode: {ok:200, error:300, timeout:301,noAuth:600,noReg:700},
	ui:{sbar:true},
	frag:{}, //page fragment
	_msg:{}, //alert message
	_set:{
	...
	loadLogin:function(){
		if ($.pdialog && Bird._set.loginTitle) {
			$.pdialog.open(Bird._set.loginUrl, "login", Bird._set.loginTitle, {mask:true,width:520,height:260,restore:false,minable:false});
		} else {
			window.location = Bird._set.loginUrl;
		}
	},
	loadReg:function(){
		if ($.pdialog && Bird._set.regTitle) {
			$.pdialog.open(Bird._set.pregUrl, "preg", Bird._set.regTitle, {mask:true,width:520,height:260,restore:false,minable:false});
		} else {
			window.location = Bird._set.pregUrl;
		}
	},
	...
	ajaxDone:function(json){
		...
		if(json.statusCode == Bird.statusCode.error || json.statusCode == Bird.statusCode.noAuth) {
			if(json.message && alertMsg) alertMsg.error(json.message);
		} else if (json.statusCode == Bird.statusCode.timeout) {
			if(alertMsg) alertMsg.error(json.message || Bird.msg("sessionTimout"), {okCall:Bird.loadLogin});
			else Bird.loadLogin();
		} else if (json.statusCode == Bird.statusCode.noReg) {
			if(alertMsg) alertMsg.error(json.message, {okCall:Bird.loadLogin});
			else Bird.loadLogin();
		} else {
	...
	init:function(pageFrag, options){
		var op = $.extend({
				loginUrl:"login.html", loginTitle:null, callback:null, debug:false, pregUrl:'preg_dialog.html',regTitle:null,
				statusCode:{}
			}, options);
		...
		this._set.pregUrl = op.pregUrl;
		this._set.regTitle = op.regTitle;
		
		$.extend(Bird.statusCode, op.statusCode);
		$.extend(Bird.pageInfo, op.pageInfo);
		
		Bird.homePath = op.homePath
		...
		jQuery.ajax({
			...
			success: function(xml){
				$(xml).find("_PAGE_").each(function(){
					...
					if (pageId) Bird.frag[pageId] = $(this).text();
				});
				
				$(xml).find("_MSG_").each(function(){
					var id = $(this).attr("id");
					if (id) Bird._msg[id] = $(this).text();
				});
...
(function($){
	...
	$.fn.extend({
				...
				success: function(response){
					...
					if (json.statusCode==Bird.statusCode.timeout){
						alertMsg.error(json.message || Bird.msg("sessionTimout"), {okCall:function(){
							...
							var viewType = op.viewType || 'navTab';
							if(viewType == 'navTab'){
								//navTab
						...
						if (alertMsg && json.message) alertMsg.error(json.message,{okCall:function(){
							var viewType = op.viewType || 'navTab';
							if(viewType == 'navTab'){
								//navTab
								navTab.closeCurrentTab();
		...
		loadUrl: function(url,data,callback,type,viewType){
			$(this).ajaxUrl({url:url, data:data,type:type ,callback:callback,viewType:viewType});
		},
		initUI: function(){
	...
	$.extend(String.prototype, {
		len:function(){ // 给string增加个len ()方法，计算string的字节数
		    return this.replace(/[^\x00-\xff]/g, "xx").length; 
		},
		isPositiveFloat:function(){//正的浮点数
			return (new RegExp(/^(([0-9]+\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[1-9][0-9]*))$/).test(this));
		},
		isNotNegativeFloat:function(){//非负的浮点数
			return (new RegExp(/^\d+(\.\d+)?$/).test(this));
		},
		isFloat:function(){//浮点数
			return (new RegExp(/^(-?\d+)(\.\d+)?$/).test(this));
		},
		...
		isNotNegativeInteger:function(){//整数或者0
			return (new RegExp(/^\d+$/).test(this));
		},
